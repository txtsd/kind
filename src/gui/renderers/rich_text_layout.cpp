...
RichTextLayout::RichTextLayout(const kind::ParsedContent& content, int width,
                               const QFont& font, int prefix_width)
    : width_(width) {
  ...
  spdlog::debug("RichTextLayout: built layout with {} spans, {} code blocks, height={}",
                span_rects_.size(), code_blocks_.size(), total_height_);
}
